        ...
        foreach (var m in list) {
            ...
            if (File.new_for_uri (m.uri).query_exists ()) {
                try {
                    ...
                    /* Async copy so the main loop stays responsive while
                     * optical media is read. Progress callback still updates
                     * the notification bar per-track. */
                    int track_index = index;
                    bool success = yield m.file.copy_async (dest, FileCopyFlags.NONE, Priority.DEFAULT, null, (current_bytes, total_bytes) => {
