...
public class Music.Plugins.AndroidLibrary : Music.Library {
    ...
    private async void import_medias_async (Gee.Collection<Music.Media> to_import) {
        ...
        foreach (var m in to_import) {
            ...
            /* Prefer the original device filename (and its real extension)
             * so GStreamer can guess a media type. Fall back to title + .mp3. */
            string dest_name = Path.get_basename (m.uri);
            int q = dest_name.last_index_of_char ('/');
            if (q >= 0) {
                dest_name = dest_name.substring (q + 1);
            }
            if (dest_name == "" || dest_name == "track") {
                dest_name = m.title ?? "track";
            }
            if (dest_name.strip () == "") {
                dest_name = "track";
            }
            ...
            string ext = "";
            int dest_dot = dest_name.last_index_of_char ('.');
            if (dest_dot > 0) {
                ext = dest_name.substring (dest_dot);
            }
            if (ext == "") {
                ext = ".mp3";
            ...
            /* Skip tracks already present in the local library (same title+artist). */
            var existing = local.find_media (m);
            if (existing != null && existing.uri.has_prefix ("file://")) {
                print ("[MTP library] Skipping duplicate %s — already in library\n", dest_name);
                index++;
                libraries_manager.progress = (double) index / total;
                continue;
            }
            ...
            if (ret != 0) {
                ...
                /* Discoverer requires a file:// URI. Passing a raw path yields
                 * "no media type was provided". Also guess/set content type. */
                var dest_file = File.new_for_path (dest_path);
                try {
                    dest_file.query_info (FileAttribute.STANDARD_CONTENT_TYPE, FileQueryInfoFlags.NONE);
                } catch (Error e) {
                    warning ("[MTP library] could not query content type: %s", e.message);
                }
                bool uncertain = false;
                string guessed = ContentType.guess (dest_path, null, out uncertain);
                if (guessed == null || guessed == "" || guessed.has_prefix ("application/octet-stream")) {
                    string mime = "audio/mpeg";
                    if (ext.down () == ".flac") {
                        mime = "audio/flac";
                    } else if (ext.down () == ".ogg" || ext.down () == ".oga") {
                        mime = "audio/ogg";
                    } else if (ext.down () == ".m4a" || ext.down () == ".aac") {
                        mime = "audio/mp4";
                    } else if (ext.down () == ".wav") {
                        mime = "audio/x-wav";
                    }
                    guessed = mime;
                }
                print ("[MTP library] content type for %s: %s\n", dest_name, guessed);

                var files = new Gee.ArrayList<string> ();
                files.add (dest_file.get_uri ());
